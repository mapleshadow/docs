...
1. 选择 `夸克TV / QuarkTV` 驱动，填写挂载路径，然后点击添加，这时候会报错，并出现乱码，没关系
2. 返回全部驱动页面，手动复制乱码，然后用其他html编辑器，还原二维码（应该是个BUG，还需要官方修复），暂停这个驱动
3. 再使用手机APP扫描二维码（如果不显示二维码，驱动右上角点击点击`表格布局` 从列表模式切换成表格模式）
4. 扫码确认后，开启驱动，即可使用
